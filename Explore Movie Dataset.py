...
clean_data.fillna('bfill',inplace=True)
...
movie_data.head(20)
movie_data.iloc[[x for x in range(20),47,48]]
...
movie_data['revenue'].groupby(movie_data['release_year']).agg('mean')
movie_data['popularity'].groupby(movie_data['director']).agg('mean').sort_values(ascending=False)
...
plt.errorbar(x=y_mean.index,y=y_mean.values,color=base_color,fmt='b-');
plt.xlabel('year')
plt.ylabel('profit')
#随着年份的增加，利润是稳步上涨的
